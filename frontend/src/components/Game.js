    ...
    <>
      ...
      <section className="text-center my-5 text-slate-800 mt-0 mb-0 max-w-md mx-auto">
        <form onSubmit={handleSubmit} className="place-items-center text-center">
          ...
          <button type="submit" disabled={input === "" || loadingResult} className={`bg-slate-400 hover:bg-slate-600 text-slate-100 border-2 border-slate-400 hover:border-slate-600 py-1 px-4 rounded inline-flex items-center relative top-2 ${input === "" || loadingResult ? 'bg-slate-300 hover:bg-slate-300 border-slate-300 hover:border-slate-300' : null}`}>
